...
from lmp_warpper import LMP_wrapper
from cap.helpers.speaker import Speaker
from cap.envs.ur5_env import UR5Env
from cap.envs.ur5_env_config import ENABLE_ROBOT, USE_MDETR
...
class UR5Demo:
    ...
    def setup_LMP(self):
        ...
        obj_name = parse_obj_name('the blocks', f'objects = {get_obj_names()}')
        say('Can you say it again, specifying where to place the {obj_name}?')
        objects = ['brown bowl', 'red block', 'brown block', 'red bowl', 'pink bowl', 'pink block']
        # grab the pink block
        obj_name = parse_obj_name('the blocks', f'objects = {get_obj_names()}')
        say('Can you say it again, specifying where to place the {obj_name}?')
        ...
        # LMP env wrapper
        cfg_tabletop['env'] = {'init_objs': self.env.object_list, 'coords': lmp_tabletop_coords}
        self.LMP_env = LMP_wrapper(self.env, cfg_tabletop)
        # creating APIs that the LMPs can interact with
        fixed_vars = {'np': np}
        fixed_vars.update({name: eval(name) for name in shapely.geometry.__all__ + shapely.affinity.__all__})
        variable_vars = {
            k: getattr(self.LMP_env, k)
            for k in ['get_bbox', 'get_obj_pos', 'get_color',
                      'is_obj_visible', 'denormalize_xy', 'put_first_on_second',
                      'get_obj_names', 'get_corner_name', 'get_side_name',]
        }
        ...
        self.lmp_tabletop_ui = LMP('tabletop_ui', cfg_tabletop['lmps']['tabletop_ui'], lmp_fgen, fixed_vars, variable_vars)

    def execute(self, user_input):
        try:
            self.lmp_tabletop_ui(user_input, f'objects = {self.env.object_list}')
...
if __name__ == '__main__':
    ...
    # ur5_demo.LMP_env.put_first_on_second('garlic', 'drawer')


    # print('object list: ', ur5_demo.obj_lst)

    # wait a sec to receive msgs. only necessary when robot movement is disabled
    # if not ENABLE_ROBOT:
    #     sleep(1)

    # user_input = 'put the {} on the {}'.format(ur5_demo.obj_lst[0], ur5_demo.obj_lst[1])
    # user_input = 'Pick up the lemon and put it in the drawer'
    # ur5_demo.execute(user_input)
    # ur5_demo.LMP_env.put_first_on_second('Rubiks cube', 'drawer')
